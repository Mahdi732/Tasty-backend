...
export const buildApiRoutes = ({ grpcClients, authMiddleware }) => {
  const router = Router();
  const getRequestId = (req) => String(req.headers['x-request-id'] || '');

  router.post('/api/v1/auth/register', async (req, res) => {
    const response = await grpcClients.user.registerUser({
  ...
  router.post('/api/v1/auth/login', async (req, res) => {
    const response = await grpcClients.user.loginUser({
      email: req.body.email,
      password: req.body.password,
      ip_address: req.ip,
      user_agent: req.headers['user-agent'] || '',
  ...
  router.get('/api/v1/auth/profile', authMiddleware, async (req, res) => {
    const response = await grpcClients.user.getUserProfile({ user_id: req.auth.userId }, {
      auth: req.auth,
      requestId: getRequestId(req),
    });
    if (!response.success) {
  ...
  router.post('/api/v1/orders', authMiddleware, requireActiveAndFaceVerified, async (req, res) => {
    const response = await grpcClients.order.createOrder({
  ...
  router.post('/api/v1/orders/:orderId/driver-arrived', authMiddleware, requireActiveAndFaceVerified, async (req, res) => {
    const response = await grpcClients.order.markDriverArrived({
  ...
  router.post('/api/v1/restaurants/:citySlug/:slug/estimate-delivery-time', async (req, res) => {
    const response = await grpcClients.restaurant.estimateDeliveryTime({
  ...
  router.post('/api/v1/face/compare-id', authMiddleware, async (req, res) => {
    const response = await grpcClients.face.compareIdWithFace({
